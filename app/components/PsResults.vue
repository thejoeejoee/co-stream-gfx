...
import {
  ...
  parseEventInput,
} from "~/utils/psResults";
import type { CategoryGroup, TeamScore, ScoredRunner } from "~/utils/psResults";

const route = useRoute();
const router = useRouter();

const initialEvent = route.query.event
  ? parseEventInput(String(route.query.event))
  : null;
...
// Group visibility (persisted in URL)
const initialGroups = route.query.groups
  ? String(route.query.groups).split(",")
  : DEFAULT_CATEGORY_GROUPS.map((g) => g.name);
const visibleGroupNames = ref<string[]>(initialGroups);

const { runners, eventInfo, isLoading, error, lastUpdate } = useOResultsPolling(
  eventId,
  interval,
);
...
const standings = computed(() => {
  return categoryGroups.value.map((group) =>
    computeGroupStandings(
      filteredRunners.value,
      group,
      2,
      globalWinPoints.value,
    ),
  );
});
...
const visibleStandings = computed(() =>
  standings.value.filter(
    (gs) =>
      ...
      visibleGroupNames.value.includes(gs.group.name),
  ),
);
...
const classOverviews = computed(() => {
  const result = new Map<
    ...
    Array<{
      className: string;
      top: typeof runners.value;
      running: typeof runners.value;
    }>
  >();
  for (const group of categoryGroups.value) {
    ...
    for (const cls of group.classes) {
      const classRunners = filteredRunners.value.filter(
        (r) => r.className === cls,
      );
      ...
      const running = classRunners.filter(
        (r) => r.startTime && !r.finishTime && r.resultStatus === null,
      );
...
function getTeamClasses(ts: TeamScore, group: CategoryGroup) {
  const scoredClasses = new Set(ts.classScores.map(cs => cs.className))
  const result: Array<{ className: string; runners: ScoredRunner[]; teamScore: number }> = [...ts.classScores]
  for (const cls of group.classes) {
    if (!scoredClasses.has(cls) && getRunningRunners(cls, ts.team).length > 0) {
      result.push({ className: cls, runners: [], teamScore: 0 })
    }
  }
  return result
}
...
function toggleGroup(name: string) {
  const idx = visibleGroupNames.value.indexOf(name);
  if (idx >= 0) {
    visibleGroupNames.value.splice(idx, 1);
  } else {
    visibleGroupNames.value.push(name);
  }
  const allVisible =
    visibleGroupNames.value.length === DEFAULT_CATEGORY_GROUPS.length;
  const query = { ...route.query };
  if (allVisible) {
    delete query.groups;
  } else {
    query.groups = visibleGroupNames.value.join(",");
  }
...
<template>
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <section v-show="configOpen" class="mb-6 border rounded p-4">
      <div class="flex flex-col gap-4">
        <div class="flex items-end gap-3">
          ...
          <UButton @click="handleLoad"> Načíst </UButton>
        </div>

        <div class="text-sm text-gray-500">
          <p class="font-medium mb-1">Kategorie:</p>
          <div class="flex flex-col gap-1">
            <label
            ...
            >
              <input
                type="checkbox"
                :checked="visibleGroupNames.includes(g.name)"
                @change="toggleGroup(g.name)"
              />
              <span>{{ g.name }}: {{ g.classes.join(", ") }}</span>
            </label>
          </div>
        </div>
    ...
    <div class="flex gap-0">
      <section
        v-for="gs in visibleStandings"
        :key="gs.group.name"
        class="flex-1 max-w-2xl px-4 border-r last:border-r-0"
      >
        <div class="flex items-center gap-3 mb-2">
          <h2 class="text-xl font-bold">
            {{ gs.group.name }}
          </h2>
          <UBadge variant="subtle"> Max bodů: {{ gs.winPoints }} </UBadge>
        </div>

        <!-- Standings header -->
        <div class="flex border-b text-left text-sm py-2 px-2">
          <span class="w-10 font-semibold">#</span>
          <span class="flex-1 font-semibold">Škola</span>
          <span class="w-16 text-right font-semibold">Body</span>
          <span class="w-20 text-right font-semibold">Čas</span>
        </div>

        <!-- Standings rows -->
        <div
          v-for="(ts, idx) in gs.standings"
          :key="ts.team"
          class="border-b hover:bg-gray-50"
        >
          <details :open="idx < 3">
            <summary class="cursor-pointer py-2 px-2 flex text-sm">
            ...
            <div class="pb-3 grid grid-cols-2 divide-x divide-gray-200">
              <div v-for="cs in getTeamClasses(ts, gs.group)" :key="cs.className" class="grid grid-cols-[1fr_auto_auto_auto] gap-x-2 text-xs px-3 tabular-nums">
                <!-- Class header row -->
                <span class="font-semibold text-gray-500 py-1">{{ cs.className }}</span>
                <span class="text-right text-gray-500 py-1">Pozice</span>
                <span class="text-right text-gray-500 py-1">Body</span>
                <span class="text-right text-gray-500 py-1">Čas</span>
                <!-- Scored runners -->
                <template v-for="sr in cs.runners" :key="sr.runner.id">
                  <span class="py-0.5 whitespace-nowrap">{{ sr.runner.name }}</span>
                  <span class="text-right py-0.5">{{ sr.position }}.</span>
                  <span class="text-right py-0.5">{{ sr.score }}</span>
                  <span class="text-right py-0.5">{{ formatTime(sr.time) }}</span>
                </template>
                <!-- Running runners -->
                <template v-for="rr in getRunningRunners(cs.className, ts.team)" :key="rr.id">
                  <span class="py-0.5 opacity-40 whitespace-nowrap">{{ rr.name }}</span>
                  <span class="text-right py-0.5 opacity-40">–</span>
                  <span class="text-right py-0.5 opacity-40">–</span>
                  <span class="text-right py-0.5 opacity-40">{{ elapsedSince(rr.startTime!) }}</span>
                </template>
              </div>
            </div>
          </details>
        ...
        <div
          v-if="classOverviews.get(gs.group.name)?.length"
          class="mt-4 text-xs space-y-2"
        >
          <div
            v-for="co in classOverviews.get(gs.group.name)"
            :key="co.className"
          >
            <span class="font-semibold">{{ co.className }}:</span>
            <span v-for="(r, i) in co.top" :key="r.id" class="ml-1">
              {{ i + 1 }}. {{ r.name }} ({{ r.club }},
              {{ formatTime(r.finishTime! - r.startTime!) }}){{
                i < co.top.length - 1 ? "," : ""
              }}
            </span>
